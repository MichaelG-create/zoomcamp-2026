-- create partitionned green trip data
CREATE TABLE IF NOT EXISTS `dtc-zoomcamp-2026.nytaxi.green_tripdata`
(
...
-- ceate external table green trip data
CREATE OR REPLACE EXTERNAL TABLE `dtc-zoomcamp-2026.green_tripdata_ext`
(
    VendorID STRING OPTIONS (description = 'A code indicating the LPEP provider that provided the record. 1= Creative Mobile Technologies, LLC; 2= VeriFone Inc.'),
    lpep_pickup_datetime TIMESTAMP OPTIONS (description = 'The date and time when the meter was engaged'),
    lpep_dropoff_datetime TIMESTAMP OPTIONS (description = 'The date and time when the meter was disengaged'),
...
-- create new staging table with unique id per row + add original file name
CREATE OR REPLACE TABLE `dtc-zoomcamp-2026.green_tripdata`
AS
SELECT
  MD5(CONCAT(
  ...
  *
FROM `dtc-zoomcamp-2026.green_tripdata_ext`;

-- merge ext table in main table
MERGE INTO `dtc-zoomcamp-2026.nytaxi.green_tripdata` T
USING `dtc-zoomcamp-2026.green_tripdata` S
