-- create partitionned yello trip data
CREATE TABLE IF NOT EXISTS `dtc-zoomcamp-2026.nytaxi.yellow_tripdata`
(
...
-- ceate external table yello trip data
CREATE OR REPLACE EXTERNAL TABLE `dtc-zoomcamp-2026.{{render(vars.table)}}_ext`
(
    VendorID STRING OPTIONS (description = 'A code indicating the LPEP provider that provided the record. 1= Creative Mobile Technologies, LLC; 2= VeriFone Inc.'),
    tpep_pickup_datetime TIMESTAMP OPTIONS (description = 'The date and time when the meter was engaged'),
    tpep_dropoff_datetime TIMESTAMP OPTIONS (description = 'The date and time when the meter was disengaged'),
...
-- create unique id per row + add original file name
CREATE OR REPLACE TABLE `dtc-zoomcamp-2026.{{render(vars.table)}}`
AS
SELECT
  MD5(CONCAT(
  ...
  *
FROM `dtc-zoomcamp-2026.{{render(vars.table)}}_ext`;

-- merge ext table in main table
MERGE INTO `dtc-zoomcamp-2026.nytaxi.yellow_tripdata` T
USING `dtc-zoomcamp-2026.{{render(vars.table)}}` S
